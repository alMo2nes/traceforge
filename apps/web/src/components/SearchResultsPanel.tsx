  ...
  return (
    <section className="results-panel panel">
      <div className="panel-header">
        <div><div className="panel-title">Search results</div><div className="panel-meta">{matches.length} matches across {selectedLogCount} selected logs</div></div>
        <span className="match-chip">{query || 'Search'}</span>
      </div>
      <div className="results-list">
        {matches.length === 0 ? <div className="empty-state">{query ? 'No matching instances in the selected logs.' : 'Enter a search term to search the selected logs.'}</div> : matches.map(({ log, node }) => (
          <button key={`${log.id}-${node.id}`} className={`match-row ${log.id === activeLogId && node.id === selectedNodeId ? 'selected' : ''}`} onClick={() => onSelectMatch(log.id, node.id)}>
            <span className={`node-icon ${node.kind}`}>{nodeIcon[node.kind]}</span>
            <span className="match-main"><span className="match-name">{node.label}</span><span className="match-context">{log.timestamp} · {log.entryPoint}{node.line ? ` · line ${node.line}` : ''}</span></span>
