...
import { flattenNodes, type InvestigationNode, type LogRecord } from './data';
        ...
        {!selectedOrg ? (
          ...
          <>
            ...
            <div className="content-grid" style={{ '--inspector-height': `${inspectorHeight}px` } as CSSProperties}>
              ...
              <section className="inspector-panel panel"><div className="panel-header inspector-header"><div><div className="panel-title">Inspector</div><div className="panel-meta">Selected node · variables, exception, or debug output</div></div>{selectedNode?.status === 'error' && <span className="error-badge">Error</span>}</div>{selectedNode && <div className="inspector-content"><div className="inspector-title-row"><span className={`node-icon large ${selectedNode.kind}`}>{nodeIcon[selectedNode.kind]}</span><div><h2>{selectedNode.label}</h2><p>{selectedNode.subtitle}</p></div></div><div className="inspector-facts"><span>Time<strong>{selectedNode.timestamp}</strong></span><span>Duration<strong>{formatDuration(selectedNode.durationMs)}</strong></span><span>Source line<strong>{selectedNode.line ?? '—'}</strong></span><span>Log<strong>{activeLog?.id ?? '—'}</strong></span></div><div className="inspector-section"><div className="section-heading">Variables & values</div><div className="variable-table">{selectedNode.variables.length === 0 ? <div className="muted-text">No variables captured at this node.</div> : selectedNode.variables.map((variable) => <div className="variable-row" key={`${variable.name}-${variable.type}`}><span className="variable-name">{variable.name}</span><span className="variable-type">{variable.type}</span><code>{variable.value}</code></div>)}</div></div><div className="inspector-section log-output-section"><div className="section-heading">Log output</div><pre className="log-output">{analysisLoading ? 'Loading investigation…' : detail}</pre></div></div>}</section>
            </div>
          </>
        )}
      </main>
