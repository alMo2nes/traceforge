...
/** Map internal scanner node types to the smaller vocabulary used by the UI. */
function kindFor(
  node: ScannerNode,
...
/** Convert nanosecond log timestamps into the compact relative value shown by TraceForge. */
...
/** Calculate a node duration from its correlated start/end events. */
...
/** Preserve important diagnostic events when a node has no larger raw-log slice. */
...
/** Extract variables declared or assigned directly under a scanner node. */
...
/** Locate the first and last physical lines belonging to a correlated node. */
...
/** Return only the raw log lines covered by the selected investigation node. */
...
function uiNode(
  ...
  // System/statement/variable nodes are intentionally hidden from the primary
  // investigation tree, but their data remains available to their parent scopes.
...
/** Scan, correlate and map one Salesforce debug log into UI-ready investigation nodes. */
